...
export async function GetConatcts(): Promise<{ data: any }> {
  try {
    const response = await fetch(`${API_URL}contacts`).then((res) =>
      res.json()
    );
    return response;
  } catch (err) {
...
export const CreateContacts = async (data: any) => {
  try {
...
export const UpdateContacts = async (id: number, data: any) => {
  try {
